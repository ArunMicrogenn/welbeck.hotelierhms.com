...
class Transaction extends CI_Controller
{
	...
	public function ReserveCheckin()
	{
		...
		rm.Resid='".$this->db->escape_str($this->input->post('resno') ? $this->input->post('resno') : $this->input->get('resno'))."' and rd.fromdate='".date('Y-m-d')."' 
	...
	public function delete_temprescheckin(){
		// BUG FIX: SQL Injection — cast count to int
		$count = (int)(isset($_REQUEST['count']) ? $_REQUEST['count'] : 0);
	...
	public function CheckoutResettlement_Val()
	{
	 ...
	 $check1 = "Select  cast(toroomid as bigint) as toroomid,totalamount,roomgrcid,checkoutdate from trans_checkout_mas where Checkoutid='" . $this->db->escape_str(isset($_REQUEST['idv']) ? $_REQUEST['idv'] : '') . "'";
...
$toroompay = '';
				if ($paymode == "COMPANY") 
				{
				...
				} else if ($paymode == "TOROOM")
				 {
					...
					$check = "select * from trans_checkout_mas cm
							...
							where Checkoutid='" . $this->db->escape_str(isset($_REQUEST['idv']) ? $_REQUEST['idv'] : '') . "'  ";
	...
	$delete = "delete Trans_Pay_Det where Checkoutid='" . $this->db->escape_str(isset($_REQUEST['idv']) ? $_REQUEST['idv'] : '') . "'";
...
Public function validatenightaudit()
{
	...
	public function Gst_State()
	{
		...
		$query = $this->db->query("exec Get_StateCountry '" . (int)$_REQUEST['Cityid'] . "'");
	...
	public function GetAdvanceGuestDetails()
	{
		...
		where Rm.Room_Id='" . (int)$_REQUEST['Roomid'] . "'
	...
	public function getdepdate()
	{
		...
		$sql = "Select det.deptime,det.depdate,det.grcid,* from Room_Status rs 
			...
			left outer  join Mas_Title mt on mt.Titleid=cus.Titelid
			Where rs.Status='Y' and isnull(billsettle,0)=0
			and rm.Room_Id=" . (int)$_REQUEST['Roomid'];
		$res = $this->db->query($sql);
		foreach ($res->result() as $row) {
	...
	public function getBlock()
	{
		...
		and roomid=" . (int)$_REQUEST['Roomid'];
		$res = $this->db->query($sql);
		foreach ($res->result() as $row) {
...
foreach ($swapqry->result_array() as $ress) {
	...
	public function Guestamendmentsave()
	{
		...
		$sql = " Select * from Mas_Room Rm
		 ...
		 where Rm.Room_Id='" . (int)$_REQUEST['Roomid'] . "' and isnull(rs.status,'')='Y' and isnull(rs.billsettle,0) = 0" ;
		$res = $this->db->query($sql);
		$Roomid = $_REQUEST['Roomid'];
		...
		foreach ($res->result_array() as $row) {
			if ($_FILES['fileToUpload']['name'][0] != '') {
				...
				// BUG FIX: SQL Injection — escape all customer update fields
				$up = "update Mas_Customer set Firstname='" . $this->db->escape_str($_REQUEST['Firstname']) . "',Middlename='" . $this->db->escape_str($_REQUEST['Middlename']) . "',Lastname='" . $this->db->escape_str($_REQUEST['Lastname']) . "',Mobile='" . $this->db->escape_str($_REQUEST['Mobile']) . "',Email_ID='" . $this->db->escape_str($_REQUEST['email']) . "',HomeAddress1='" . $this->db->escape_str($_REQUEST['address1']) . "',HomeAddress2='" . $this->db->escape_str($_REQUEST['address2']) . "',HomeAddress3='" . $this->db->escape_str($_REQUEST['address3']) . "',Cityid='" . (int)$_REQUEST['Cityid'] . "',Nationality='" . (int)$_REQUEST['Nationid'] . "',ResidentialPhone='" . $this->db->escape_str($_REQUEST['Phone']) . "',WorkAddress1='" . $this->db->escape_str($_REQUEST['workaddress1']) . "',
				WorkAddress2='" . $this->db->escape_str($_REQUEST['workaddress2']) . "',WorkAddress3='" . $this->db->escape_str($_REQUEST['workaddress3']) . "',Id_Documenttype='" . $this->db->escape_str($_REQUEST['Id_Documenttype']) . "',Id_Documentno='" . $this->db->escape_str($_REQUEST['Id_Documentno']) . "', Photopath = '" . $path . "', Titelid = '".(int)$_REQUEST['title']."'
					where Customer_Id='" . (int)$row['Customerid'] . "'";
				}
	
				// BUG FIX: SQL Injection — escape all customer update fields (no photo path version)
				$up = "update Mas_Customer set Firstname='" . $this->db->escape_str($_REQUEST['Firstname']) . "',Middlename='" . $this->db->escape_str($_REQUEST['Middlename']) . "',Lastname='" . $this->db->escape_str($_REQUEST['Lastname']) . "',Mobile='" . $this->db->escape_str($_REQUEST['Mobile']) . "',Email_ID='" . $this->db->escape_str($_REQUEST['email']) . "',HomeAddress1='" . $this->db->escape_str($_REQUEST['address1']) . "',HomeAddress2='" . $this->db->escape_str($_REQUEST['address2']) . "',HomeAddress3='" . $this->db->escape_str($_REQUEST['address3']) . "',Cityid='" . (int)$_REQUEST['Cityid'] . "',Nationality='" . (int)$_REQUEST['Nationid'] . "',ResidentialPhone='" . $this->db->escape_str($_REQUEST['Phone']) . "',WorkAddress1='" . $this->db->escape_str($_REQUEST['workaddress1']) . "',
				WorkAddress2='" . $this->db->escape_str($_REQUEST['workaddress2']) . "',WorkAddress3='" . $this->db->escape_str($_REQUEST['workaddress3']) . "',Id_Documenttype='" . $this->db->escape_str($_REQUEST['Id_Documenttype']) . "',Id_Documentno='" . $this->db->escape_str($_REQUEST['Id_Documentno']) . "',Titelid = '".(int)$_REQUEST['title']."'
					where Customer_Id='" . (int)$row['Customerid'] . "'";
		...
		$sql5 = "select dbo.Get_Taxid('" . $RoomRent . "','" . $RoomRent . "','" . $RoomRent . "','" . $guestcharge . "','" . $this->db->escape_str(isset($_REQUEST['discamt']) ? $_REQUEST['discamt'] : '0') . "',(select DateofAudit from Night_audit)) as Taxid";
		$res5 = $this->db->query($sql5);
		foreach ($res5->result_array() as $row5) {
			$Taxid = $row5['Taxid'];
		}
...
$Bankiddd  = $_REQUEST['bank'];
	...
	public function ResCheckinsave()
	{
		...
		for ($i = 0; $i < $rowcount; $i++) {
			...
			where mas.ResNo='" . $_REQUEST['ResNo'] . "' and det.typeid='" . (int)(isset($_REQUEST['roomtype'][$i]) ? $_REQUEST['roomtype'][$i] : 0) . "' and det.fromdate = '" . $Indate . "' ";
			$residarr = $this->db->query($residquery);
			$resid = $residarr->row_array(); 

			$che = " select isnull(checkinrooms,0) as checkinrooms, * from  trans_reserve_det trdet inner join trans_reserve_mas trmas on trmas.resid = trdet.resid  where trmas.resno='" . $this->db->escape_str(isset($_REQUEST['ResNo']) ? $_REQUEST['ResNo'] : '') . "' and trdet.fromdate = '" . $Indate . "' and typeid='" . (int)(isset($_REQUEST['roomtype'][$i]) ? $_REQUEST['roomtype'][$i] : 0) . "'";
			...
			// inner join Trans_Reserve_det det on mas.Resid=det.resid 
			// where mas.ResNo='" . $_REQUEST['ResNo'] . "' and det.typeid='" . (int)(isset($_REQUEST['roomtype'][$i]) ? $_REQUEST['roomtype'][$i] : 0) . "') and typeid='" . (int)(isset($_REQUEST['roomtype'][$i]) ? $_REQUEST['roomtype'][$i] : 0) . "' ";
			...
			$sql3 = "SELECT * FROM Mas_Room rm
				Inner join Mas_Roomtype rt on rt.RoomType_Id=rm.RoomType_Id
				where rm.RoomType_Id='" . (int)(isset($_REQUEST['roomtype'][$i]) ? $_REQUEST['roomtype'][$i] : 0) . "'";
			$res3 = $this->db->query($sql3);
			foreach ($res3->result_array() as $row3) {
			...
			'" . $_REQUEST['travelagent_Id'] . "','" . $_REQUEST['bookingid'][$i] . "','" . $_REQUEST['discper'] . "','" . $this->db->escape_str(isset($_REQUEST['discamt']) ? $_REQUEST['discamt'] : '0') . "','" . $_REQUEST['foodplan'][$i] . "','0','" . $_REQUEST['Adults'][$i] . "')";
		...
		if ($result) {
			...
			for ($i = 0; $i < $rowcount; $i++) {
			 ...
			 if($roomtypeid  != $_REQUEST['roomtype'][$i]) 
			 {
			    ...
			    $reschek ="select count(*) as count,noofrooms from temp_trans_reservecheckin_log where typeid='" . (int)(isset($_REQUEST['roomtype'][$i]) ? $_REQUEST['roomtype'][$i] : 0) . "' 
			...
			where Billid=(select det.Billid from trans_reserveadd_mas mas 
				...
				where mas.resno ='" . $this->db->escape_str(isset($_REQUEST['ResNo']) ? $_REQUEST['ResNo'] : '') . "' )";
...
// ------Power----
	...
	public function ReserveAmendmentsave()
	{
		...
		travelagentid='" . $_REQUEST['travelagent_Id'] . "',onlinebookingno='" . $_REQUEST['bookingid'] . "',totalrent='" . $RoomRent . "',age='',discper='" . $_REQUEST['discper'] . "',discount='" . $this->db->escape_str(isset($_REQUEST['discamt']) ? $_REQUEST['discamt'] : '0') . "',muserid='" . User_id . "',Roomid='" . $_REQUEST['Roomid'] . "' where resid='" . $_REQUEST['Resid'] . "'";
	...
	public function Reservesave()
	{
		...
		$ins = "Insert into Trans_reserve_mas(ResNo,Reserveby,Communicate,Resdate,cusid,totamt,totrooms,stat,userid,waitlist,duedate,travelagentid,onlinebookingno,totalrent,age,discper,discount,muserid,Roomid,isconfirm)
	       values(dbo.ResNo(),'" . User_id . "','P','" . $date . "','" . $Customer_Id . "','" . $RoomRent . "','1','','" . User_id . "','0','" . $date . "','" . $_REQUEST['travelagent_Id'] . "','" . $_REQUEST['bookingid'] . "','" . $RoomRent . "','','" . $_REQUEST['discper'] . "','" . $this->db->escape_str(isset($_REQUEST['discamt']) ? $_REQUEST['discamt'] : '0') . "','" . User_id . "','" . $_REQUEST['Roomid'] . "',1)";
...
public function Collection_Val()
{
	...
	public function Reservation_save()
	{
	  ...
	  values(dbo.ResNo(),'" . User_id . "','P','" . $date . "','" . $Customer_Id . "','" . $_REQUEST['roomcount'] . "','','" . User_id . "','" . $_REQUEST['ReservationMode'] . "','" . $_REQUEST['duedate'] . "','" . $_REQUEST['travelagent_Id'] . "','" . $_REQUEST['bookingid'] . "','','" . $_REQUEST['discper'] . "','" . $this->db->escape_str(isset($_REQUEST['discamt']) ? $_REQUEST['discamt'] : '0') . "','" . User_id . "',
	...
	public function ReservationEdit_save()
{
    ...
    $transactionSql .= "INSERT INTO Trans_reserve_mas(ResNo,Reserveby,Communicate,Resdate,cusid,totrooms,stat,userid,waitlist,duedate,travelagentid,onlinebookingno,age,discper,discount,muserid,company,yearprefix)
        VALUES ('" . $_REQUEST['resno'] . "','" . User_id . "','P','" . $date . "','" . $Customer_Id . "','" . $_REQUEST['roomcount'] . "','','" . User_id . "','" . $mode['ReservationMode_Id'] . "','" . $_REQUEST['duedate'] . "','" . $_REQUEST['travelagent_Id'] . "','" . $_REQUEST['bookingid'] . "','','" . $_REQUEST['discper'] . "','" . $this->db->escape_str(isset($_REQUEST['discamt']) ? $_REQUEST['discamt'] : '0') . "','" . User_id . "','" . $_REQUEST['CompanyId'] . "',dbo.YearPrefix());\n";
...
foreach ($checkqry->result_array() as $ress) {
   ...
   // Reservation amendment validation and update
	...
	public function ReservationEditold_save()
	{
		...
		values('" . $_REQUEST['resno'] . "','" . User_id . "','P','" . $date . "','" . $Customer_Id . "','" . $_REQUEST['roomcount'] . "','','" . User_id . "','" . $_REQUEST['ReservationMode'] . "','" . $_REQUEST['duedate'] . "','" . $_REQUEST['travelagent_Id'] . "','" . $_REQUEST['bookingid'] . "','','" . $_REQUEST['discper'] . "','" . $this->db->escape_str(isset($_REQUEST['discamt']) ? $_REQUEST['discamt'] : '0') . "','" . User_id . "',
