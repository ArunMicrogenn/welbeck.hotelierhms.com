...
$this->pweb->Cheader('Report','Guest Checkin Details');
$this->pfrm->FrmHead6('Report /Guest Checkin Details',$F_Class."/".$F_Ctrl,$F_Class."/".$F_Ctrl."_View");
  ...
  <div class="the-box D_IS" ></div>
    ...
    <div id="printing" class="col-sm-12">
        ...
        <table class="table table-bordered table-hover"  >  
		    <div>
				<h4 class="text-center">Guest Checkin Details  <?php echo date('d-m-Y', strtotime($_POST['frmdate'])); ?> To <?php echo date('d-m-Y', strtotime($_POST['todate'])); ?><h4>
		    </div>           
		   <tbody>
		    <?php 
			 $i=1;	
             ...
             $qry="exec Guest__Checkin__Details '".$fromdate."', '".$todate."'";
			 ...
			 $advance= $exec->num_rows();
			  if($advance !=0)
			  {
                ...
                echo '<td style="text-align: center;">Address1</td>';
                // echo '<td style="text-align: center;">Address2</td>';
                echo '<td style="text-align: center;">City</td>';
                echo '<td style="text-align: center;">Contact no</td>';
				echo '<td  style="text-align: center;">User</td>';
				// echo '<td style="text-align: center;">ID Proof</td>';	
                // echo '<td style="text-align: center;">ID ProofNo</td>';
				echo '</tr>';	
			  }			
			  foreach ($exec->result_array() as $rows)
			  {
                ...
                echo '<td style="text-align: right;">'.$rows['male'].'</td>';
                echo '<td style="text-align: right;">'.$rows['female'].'</td>';              
                echo '<td style="text-align: center;">'.$rows['HomeAddress1'].'</td>';
                // echo '<td style="text-align: left;">'.$rows['homeaddress2'].'</td>';
				...
				// echo '<td style="text-align: right;">'.$rows['documenttype'].'</td>';	
                // echo '<td style="text-align: center;">'.$rows['documentno'].'</td>';			
			  }
			//   if($advance !=0)
			//   {
			// 	echo '<tr>';
        ...
        $("#exporttable").click(function(e)
		{
          ...
          if(table && table.length)
		  {
            $(table).table2excel({
              ...
              filename: "Guest Checkin Details" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
              fileext: ".xls",
              exclude_img: true,
              exclude_links: true,
              exclude_inputs: true,
        ...
        $("body").on("click", "#exportpdf", function () {
            html2canvas($('#printing')[0], {
                onrendered: function (canvas) {
                    ...
                    pdfMake.createPdf(docDefinition).download("Guest Checkin Details.pdf");
                }
            });
        });
	</SCRIPT>
