  ...
  <div class="the-box F_ram">
	...
	<form action="" method="POST">
        ...
        <tr>
          ...
          <td align="left"><input type="date"  id="todate" name="Tdate" value="<?php echo date('Y-m-d'); ?>"  class="scs-ctrl " />
            <div class="Type" ></div></td>        
...
<div id="printing" class="col-sm-12" style="overflow: scroll; height: 400px;">
  <?php
    if(@$_POST['submit'])
		{
			?>
        ...
        <div>
				<h4 class="text-center">Reservation Details  <?php echo date('d-m-Y', strtotime($_POST['Fdate'])); ?> To <?php echo date('d-m-Y', strtotime($_POST['Tdate'])); ?><h4>
		    </div>        
		   <tbody>
		    <?php 
            ...
            //  }
		       ...
		       $qry="select tmas.Amount as advanceamount,mas.yearprefix,mas.cusid,cus.firstname as customer,mt.Title, * from Trans_reserve_mas mas 
			  ...
			  inner join mas_reservationmode mrm on mrm.ReservationMode_Id = mas.waitlist
       ...
       $advanceamt=0;
			  if($advance !=0)
			  {
				...
				echo '<td style="text-align: center;">No Of Rooms</td>';
			  ...
			  $userqry = "select EmailId from usertable where User_id = '".User_id."'";
			  
			  $exuser = $this->db->query($userqry)->row_array();
			  ...
			  {		
				...
				// $cust = "select * from mas_customer wher customer_id = "''"";
                $time= new dateTime($rows['fromtime']);	
                $totime= new dateTime($rows['totime']);	
                
                if($rows['advance'] !=NULL){
                  $advanceamt=$rows['advance'];
                }
				echo '<tr>';		 
				...
				echo '<td  style="text-align: center;">'.date('d-m-Y', strtotime(substr($rows['todate'],0,10))).'</td>';
                echo '<td  style="text-align: center;">'.$totime->format('H:i').'</td>';
                echo '<td  style="text-align: center;">'.$rows['PrintingName'].'</td>';
                echo '<td  style="text-align: center;">'.$rows['Title'].'.'.$rows['customer'].'</td>';
                echo '<td  style="text-align: center;">'.$rows['noofrooms'].'</td>';
                echo '<td  style="text-align: center;">'.$rows['noofpax'].'</td>';
                echo '<td  style="text-align: right;">'.number_format($rows['totalroomrent'],2).'</td>';
                echo '<td  style="text-align: right;">'.number_format($advanceamt,2).'</td>';
					echo '<td  style="text-align: left;"> '.$rows['ReservationMode'].'</td>';
