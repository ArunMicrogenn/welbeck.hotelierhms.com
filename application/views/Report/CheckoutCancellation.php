...
$this->pweb->Cheader('Report','Checkout Cancellation');
$this->pfrm->FrmHead6('Report / Checkout Cancellation',$F_Class."/".$F_Ctrl,$F_Class."/".$F_Ctrl."_View");
  ...
  <div class="the-box D_IS" ></div>
    ...
    <div id="printing" class="col-sm-12">
        ...
        <table class="table table-bordered table-hover"  >  
		    <div>
				<h4 class="text-center">Checkout Cancellation Report <?php echo date('d-m-Y', strtotime($_POST['frmdate'])); ?> To <?php echo date('d-m-Y', strtotime($_POST['todate'])); ?><h4>
		    </div>           
		   <tbody>
		    <?php 
			 $i=1;		 
            ...
            isnull(mcom.company,' ') as company,mas.reason,u.Emailid from trans_checkout_mas mas
            ...
            where mas.Checkoutdate between '".date('Y-m-d', strtotime(@$_POST['frmdate']))."' and '".date('Y-m-d', strtotime(@$_POST['todate']))."' and 
             isnull(mas.cancelflag,0)<>0  and mas.checkoutno like 'CHK%'
            order by mas.checkoutno";
			 ...
			 $advance= $exec->num_rows();
			  ...
			  foreach ($exec->result_array() as $rows)
			  {
				...
				echo '<td style="text-align: center;">'.$rows['Title'].'.'.$rows['Guestname'].'</td>';
				...
				echo '<td style="text-align: center;">'.$rows['reason'].'</td>';
        ...
        $("#exporttable").click(function(e)
		{
          ...
          if(table && table.length)
		  {
            $(table).table2excel({
              ...
              filename: "Checkout Cancellation Report" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
              fileext: ".xls",
              exclude_img: true,
              exclude_links: true,
              exclude_inputs: true,
        ...
        $("body").on("click", "#exportpdf", function () {
            html2canvas($('#printing')[0], {
                onrendered: function (canvas) {
                    ...
                    pdfMake.createPdf(docDefinition).download("Checkout Cancellation Report.pdf");
                }
            });
        });
	</SCRIPT>