...
$this->pweb->Cheader('Report','Guest Expected Depature Report');
$this->pfrm->FrmHead6('Report /Guest Expected Depature Report',$F_Class."/".$F_Ctrl,$F_Class."/".$F_Ctrl."_View");
  ...
  <div class="the-box D_IS" ></div>
    ...
    <div id="printing" class="col-sm-12">
        ...
        <table class="table table-bordered table-hover"  >  
		    <div>
				<h4 class="text-center">Today's Depature Report  <?php echo date('d-m-Y', strtotime($_POST['frmdate'])); ?> To <?php echo date('d-m-Y', strtotime($_POST['todate'])); ?><h4>
		    </div>           
		   <tbody>
		    <?php 
			 $i=1;	
			 ...
			 $advance= $exec->num_rows();
			  if($advance !=0)
			  {
                ...
                echo '<td style="text-align: center;">out Time</td>';
			  ...
			  foreach ($exec->result_array() as $rows)
			  {
                ...
                echo '<td style="text-align: center;">'.date('H:i',strtotime($rows['Checkouttime'])).'</td>';                
        ...
        $("#exporttable").click(function(e)
		{
          ...
          if(table && table.length)
		  {
            $(table).table2excel({
              ...
              filename: "Departure__Report" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
              fileext: ".xls",
              exclude_img: true,
              exclude_links: true,
              exclude_inputs: true,
        ...
        $("body").on("click", "#exportpdf", function () {
            html2canvas($('#printing')[0], {
                onrendered: function (canvas) {
                    ...
                    pdfMake.createPdf(docDefinition).download("Departure__Report.pdf");
                }
            });
        });
	</SCRIPT>