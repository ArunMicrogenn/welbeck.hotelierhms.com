...
$this->pweb->Cheader('Report','BillEntry Details');
$this->pfrm->FrmHead6('Report / BillEntry Details',$F_Class."/".$F_Ctrl,$F_Class."/".$F_Ctrl."_View");
  ...
  <div class="the-box D_IS" ></div>
    ...
    <div id="printing" class="col-sm-12">
        ...
        <table class="table table-bordered table-hover"  >  
		    <div>
				<h4 class="text-center">Bill Entry Report  <?php echo date('d-m-Y', strtotime($_POST['frmdate'])); ?> To <?php echo date('d-m-Y', strtotime($_POST['todate'])); ?><h4>
		    </div>           
		   <tbody>
		    <?php 
			 $i=1;		 
			$qry="select * from Mas_Revenue re
			 ...
			 where CreditDate between '".date('Y-m-d',strtotime($_POST['frmdate']))."' and '".date('Y-m-d',strtotime($_POST['todate']))."'  and isnull(Taxhead,'') not in('ROOM','CGST','SGST','ADVANCE','Discount')";
			 ...
			 $advance= $exec->num_rows();
			  if($advance !=0)
			  {
				...
				echo '<td style="text-align: center;">Credit.No</td>';
			  ...
			  foreach ($exec->result_array() as $rows)
			  {
				...
				echo '<td style="text-align: center;">'.$rows['RevenueHead'].'</td>';
        ...
        $("#exporttable").click(function(e)
		{
          ...
          if(table && table.length)
		  {
            $(table).table2excel({
              ...
              filename: "Bill Entry Report" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
              fileext: ".xls",
              exclude_img: true,
              exclude_links: true,
              exclude_inputs: true,
        ...
        $("body").on("click", "#exportpdf", function () {
            html2canvas($('#printing')[0], {
                onrendered: function (canvas) {
                    ...
                    pdfMake.createPdf(docDefinition).download("BillEntryDetails.pdf");
                }
            });
        });
