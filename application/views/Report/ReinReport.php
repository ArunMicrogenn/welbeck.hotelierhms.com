...
$this->pweb->Cheader('Report','Noshows Report');
$this->pfrm->FrmHead6('Report /Noshows Report',$F_Class."/".$F_Ctrl,$F_Class."/".$F_Ctrl."_View");
  ...
  <div class="the-box F_ram">
	...
	<form action="" method="POST">
        ...
        <tr>
          ...
          <td align="left"><input type="date" value="<?php echo date('Y-m-d'); ?>" id="frmdate" name="frmdate" max="<?php echo date('Y-m-d'); ?>"  class="scs-ctrl Dat2" />
            <div class="Type" ></div></td>
            <td align="right" class="F_val">To Date</td>
          <td align="left"><input type="date" value="<?php echo date('Y-m-d'); ?>" id="todate" name="todate" max="<?php echo date('Y-m-d'); ?>"  class="scs-ctrl Dat2" />
            <div class="Type" ></div></td>        
		    ...
		    <?php 
			 ...
			 $advance= $exec->num_rows();
			  if($advance !=0)
			  {
                ...
                echo '<td style="text-align: center;">Total Rooms</td>';
			  ...
			  foreach ($exec->result_array() as $rows)
			  {
				...
				echo '<td style="text-align: right;">'.$rows['totrooms'].'</td>';
				echo '<td style="text-align: center;">'.$rows['company'].'</td>';
                echo '<td style="text-align: center;">'.$rows['onlinebookingno'].'</td>';
