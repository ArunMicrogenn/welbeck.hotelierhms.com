...
<div id="printing"  class="col-sm-12" style="overflow:scroll;height:430px;">
		...
		<div>
		   ...
		   <tbody>
		    <?php 
			 ...
			 $advance= $exec->num_rows();
			  ...
			  {
				echo '<tr>';
				echo '<td colspan="10" class="text-bold" style="text-align: center;background-color:#c9c6c6;">Advance Collection</td>';			
				echo '</tr>';
			  ...
			  foreach ($exec->result_array() as $rows)
			  {
				...
				echo '<td style="text-align: center;text-wrap:nowrap">'.date('d-m-Y',strtotime($rows['rptdate'])).'</td>';
		   ...
		   ('CHK%') and isnull(tpd.Paymodeid,0)<>18 and isnull(tcm.groupcheckout,0) = 0 and
			...
			and isnull(tpd.Paymodeid,0)<> 4 order by tcm.Checkoutno desc ";
			 ...
			 $checkout= $exec3->num_rows(); $i=1;
			  ...
			  {
				echo '<tr>';
				echo '<td class="text-bold" colspan="10" style="text-align: center;background-color:#c9c6c6;">Checkout Bills</td>';			
				echo '</tr>';			  
				echo '<tr  style="background-color:#c9c6c6">';		 
				echo '<td  style="text-align: center;">S.No</td>';
				echo '<td  style="text-align: center;">Receipt.No</td>';
			  ...
			  $credittotal=0;
			  $TotalCheckout = 0;
			  $shown_group_1 = false; // <-- ADD THIS BEFORE THE foreach()
		   ...
		   ('CHK%') and isnull(tpd.Paymodeid,0)<>18 and isnull(tcm.groupcheckout,0) = 1 and
			tcm.Checkoutdate between '".date('Y-m-d',strtotime($_POST['frmdate']))."' 
			and '".date('Y-m-d',strtotime($_POST['todate']))."' and tcm.settle <>0 and isnull(tcm.cancelflag,0)=0
			and isnull(tpd.Paymodeid,0)<> 4 order by tcm.Checkoutid desc ";
			 $exec3=$this->db->query($qry3); $totalAdvance=0;
			 $checkout= $exec3->num_rows(); $i=1;
			  if($checkout !=0)
			  {
				echo '<tr>';
				echo '<td class="text-bold" colspan="10" style="text-align: center;background-color:#c9c6c6;">Group Checkout Bills</td>';			
				echo '</tr>';			  
				echo '<tr  style="background-color:#c9c6c6">';		 
				echo '<td  style="text-align: center;">S.No</td>';
				echo '<td  style="text-align: center;">Receipt.No</td>';
			  ...
			  $checkout= $exec3->num_rows(); $i=1;
			   ...
			   $sql1 = "select mas.Checkoutno,mas.yearPrefix,mc.company as paymode from trans_bill_det  det
				...
				echo '<td  style="text-align: center;">'.$i++.'</td>';
				echo '<td  style="text-align: center;" colspan="2">'.$rows3['Creditno'].'</td>';
