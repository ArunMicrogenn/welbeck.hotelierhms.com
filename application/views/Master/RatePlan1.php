...
<script>
 ...
 function Rate_Set()
 {
    ...
    var formData = {
        ...
        PlanType_Id: $('#PlanType_Id').val(),
        FromDate: $('[name="FromDate"]').val(),
        ToDate: $('[name="ToDate"]').val(),
        Single: $('#Single').val() || '0',
        Double: $('#Double').val() || '0',
        Triple: $('#Triple').val() || '0',
        Quadruple: $('#Quadruple').val() || '0',
        AdultRate: $('#AdultRate').val() || '0',
        ChildRate: $('#ChildRate').val() || '0',
        FoodPlan_Id: $('#FoodPlan_Id').val() || '0',
        AdultPlanAmt: $('#AdultPlanAmt').val() || '0',
        ChildPlanAmt: $('#ChildPlanAmt').val() || '0',
        Keey: $('#Keey').val(),
        idv: $('[name="idv"]').val()
    };
    ...
    $.ajax({
        ...
        error: function(xhr, status, error) {
            ...
            alert('Error during validation. Please check all fields.');
        }
    });
 }

 function Rate_SetII(formData)
 {
    $.ajax({
        type: "POST",
        ...
        success: function(response) {
            if(response.success) {
                ...
                alert(response.message || 'Failed to save rate plan');
            }
        },
        error: function(xhr, status, error) {
            console.log('Error:', error);
            console.log('Server response:', xhr.responseText);
            alert('Error saving rate plan. Please check all fields.');
        }
    });
 }
 function Delete_(ee)
