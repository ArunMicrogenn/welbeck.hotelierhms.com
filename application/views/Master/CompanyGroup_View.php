		 ...
		 foreach($Res as $row)
			{
               ...
               echo  '<tr class="odd gradeX">
                      <td>'.$count.' </td>
                      <td>'.$row['CompanyGroup'].'</td>';    ?>
                      <td style="color:<?php if($row['Active']==0){ echo "Green";}else{echo "Red";} ?>"><?php if($row['Active']==0){ echo "Is Active";}else{echo "Is Inactive";} ?></td>
				<?php					   		 
               echo  '<td>
					  <a class="btn btn-warning btn-xs" href="'.scs_index.$F_Class."/".$F_Ctrl."/".$row['CompanyGroup_Id'].'/UPDATE"   >
					  <i class="fa fa-edit"></i> Edit</a> | 
					   <a class="btn btn-danger btn-xs" href="'.scs_index.$F_Class."/".$F_Ctrl."/".$row['CompanyGroup_Id'].'/DELETE"   >
