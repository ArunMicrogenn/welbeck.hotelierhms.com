...
<p style="font-size:13px; margin-top:12px;"><strong>Room Details : -</strong></p>
<div style="height: 10vh; overflow-y: auto; ">
  <table class="FrmTable" id="roomTable" style="width: 100%; border-collapse: collapse;">
    <thead>
      <tr>
        <td style="position: sticky; top: 0; background: #FFFAA0; z-index: 2;">Room No</td>
        <td style="position: sticky; top: 0; background: #FFFAA0; z-index: 2;">Rate Type</td>
        <td style="position: sticky; top: 0; background: #FFFAA0; z-index: 2;">Male</td>
        <td style="position: sticky; top: 0; background: #FFFAA0; z-index: 2;">Female</td>
        <td style="position: sticky; top: 0; background: #FFFAA0; z-index: 2;">Child</td>
        <td style="position: sticky; top: 0; background: #FFFAA0; z-index: 2;">Pax</td>
        <td style="position: sticky; top: 0; background: #FFFAA0; z-index: 2;">Tariff</td>
        <td style="position: sticky; top: 0; background: #FFFAA0; z-index: 2;">Action</td>
      </tr>
    </thead>
    ...
    <tbody>
      <tr data-row="1">
        <td>
          <select style="width: 100px;" class="m-ctrl" name="RoomNo_1" id="RoomNo_1">
			<?php $roomqry = "select mr.Room_Id,mr.RoomNo from mas_room mr
   inner join room_status rs on mr.Room_Id = rs.roomid
   where isnull(rs.status,'N')='N' and isnull(notready,0)=0 and isnull(rs.mblock,0)=0 and isnull(rs.foblock,0)=0 and isnull(mr.inactive,0) =0";
   $rom = $this->db->query($roomqry)->result_array();?>
   <?php foreach($rom as $rom) {?>
	<option 
    value="<?php echo $rom['Room_Id']; ?>" 
    <?php if ($rom['Room_Id'] == $Roomid) echo 'selected'; ?>>
    <?php echo $rom['RoomNo']; ?>
</option>
		  <?php } ?>
          </select>
        ...
        <td>
          <select style="width: 150px;" class="m-ctrl" name="RateCode_1" id="RateCode_1" onchange="getratetyperoomrent(this.value)">
        ...
        <td>
          <select style="width: 100px;" onchange="Adult(this)" name="male_1" id="male_1" class="m-ctrl">
            <option selected value='0'>0</option>
            <?php
              $pax = $Adults + $Extrabedcount;
              for ($i = 1; $i <= $pax; $i++) {
                echo '<option value="'.$i.'">'.$i.'</option>';
              }
            ?>
        ...
        <td>
          <select style="width: 100px;" onchange="Adult(this)" name="female_1" id="female_1" class="m-ctrl">
            <option selected value='0'>0</option>
            <?php
              for ($i = 1; $i <= $pax; $i++) {
                echo '<option value="'.$i.'">'.$i.'</option>';
	 ...
	 <input type='hidden' name='totalocc' id='totalocc' value="<?php echo $totalpax; ?>">
          <select style="width: 100px;" onchange="Adult(this)" name="child_1" id="child_1" class="m-ctrl">
            <?php for ($i = 0; $i <= 3; $i++) { echo '<option value="'.$i.'">'.$i.'</option>'; } ?>
          </select>
        </td>
        <td>
          <input  type="text" id="Adults_1" name="Adults_1" class="m-ctrl">
        </td>

        <td>
          <a class="btn btn-warning btn-sm tarifpop">DT</a>
        </td>

        <td style="display:flex; gap:2px;">
          <button type="button" class="addRow btn btn-success btn-sm " >+</button>
...
<table class="FrmTable" style="margin-top:-15px" >
  ...
  <tr>
    <td>
		<input name="Mobile" id="Mobile" placeholder="Mobile"  oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="m-ctrl" >
	</td>
...
<table class="FrmTable" >
 ...
 <td>Address 1</td>
  ...
  <td>
	<!-- <input type="text" placeholder="City" id="City1" name="City" class="m-ctrl " /> -->

	     <select id="City1" name="City" onchange="getcity(this.value)">
...
function Niight(){
	...
	var planidd = document.getElementById("RateCode_" + count).value 
...
let lastRow  = $("#roomTable tbody tr:last");
let firstRow = $("#roomTable tbody tr:first");

let roomNo   = lastRow.find("select[name^='RoomNo']").val();
let rateType = lastRow.find("select[name^='RateCode']").val();
let male     = lastRow.find("select[name^='male']").val();
let female   = lastRow.find("select[name^='female']").val();
let child    = lastRow.find("select[name^='child']").val();
let adults   = lastRow.find("input[name^='Adults']").val();

if (roomNo === "0" || !roomNo) {
	swal("Incomplete Details!", "Please fill Room No", "warning");
	return;
}
if (rateType === "0" || !rateType) {
	swal("Incomplete Details!", "Please fill Rate Type", "warning");
	return;
}
if ((male === "0" || !male) && (female === "0" || !female) && (child === "0" || !child)) {
	swal("Incomplete Details!", "Please fill Guest Details", "warning");
	return;
}
...
// ------------------------------------------------------
// 🔥 GET ALL ALREADY SELECTED ROOM NUMBERS
// ------------------------------------------------------
let usedRooms = [];
...
// ------------------------------------------------------
// 🔥 REMOVE ALL USED ROOMS FROM NEW ROW DROPDOWN
// ------------------------------------------------------
usedRooms.forEach(function (rm) {
	newRow.find("select[name^='RoomNo'] option[value='" + rm + "']").remove();
});


// Reset fields
newRow.find("select[name^='RoomNo']").val("0");
newRow.find("select[name^='RateCode']").val("0");
newRow.find("select[name^='male']").val("0");
newRow.find("select[name^='female']").val("0");
newRow.find("select[name^='child']").val("0");
newRow.find("input[name^='Adults']").val("1");

newRow.find("input, select").css("border", "");
...
function renumberRows() {
	$("#roomTable tbody tr").each(function (index) {
		...
		$(this).find("input, select").each(function () {
			...
			document.getElementById("count").value = rowNumber;

			if (oldId) $(this).attr("id", oldId.replace(/_\d+$/, "_" + rowNumber));
			if (oldName) $(this).attr("name", oldName.replace(/_\d+$/, "_" + rowNumber));
		});
	});
}
...
function CheckIn() {
    if (currentdate <= InDate) {
        alert('Unable to do Check-In at Future Date. Use Reservation Option');
        return;
    }
    ...
    var discamt = document.getElementById("discamt").value;
    var foodplan = document.getElementById("foodplan").value;
    var rmtype = document.getElementById("roomtypeIdval").value;


	
	$.ajax({
    url: "<?php echo scs_index ?>Transaction/getroomtypeextraamount",
    type: "POST",
    dataType: "json",
    data: {
        roomid: Roomid
    ...
    success: function(response) {
		var extracnt =   response[0].Extrabedcount;
    ...
    rows.forEach(row => {
        var roomNo = row.querySelector('select[name^="RoomNo_"]').value;
        var rateCode = row.querySelector('select[name^="RateCode_"]').value;
        var male = row.querySelector('select[name^="male_"]').value;
        var female = row.querySelector('select[name^="female_"]').value;
        var child = row.querySelector('select[name^="child_"]').value;
        var adults = row.querySelector('input[name^="Adults_"]').value;
    ...
    let totalAdultss = roomData.reduce((sum, r) => sum + Number(r.adults || 0), 0);
	
   if(response[0].adults < totalAdultss){
	if(response[0].totalcount <= totalAdultss){  
			if(response[0].amount <= 0 ) {
            alert("Extra Bed Amount is 0");
			return;
			}
		 }
   }
...
$("#roomTable tbody tr").each(function (index) {
    ...
    let roomNo   = $("#RoomNo_" + rowIndex).val();
    let rateType = row.find("select[name^='RateCode']").val();
    let male     = row.find("select[name^='male']").val();
    let female   = row.find("select[name^='female']").val();
    let child    = row.find("select[name^='child']").val();
    let adults   = row.find("input[name^='Adults']").val();

    if (!roomNo) {
        isValid = false;
        missingFields += `Room No missing `;
    }
    if (!rateType) {
        isValid = false;
        missingFields += `Rate Type missing`;
    }
     ...
     if ((male === "0" || !male) && (female === "0" || !female) && (child === "0" || !child)) {
        isValid = false;
        missingFields = "Please Fill the Pax";
...
if (!isValid) {
        swal({
            ...
            text: "Please fill " + missingFields,
            icon: "warning",
            button: "OK",
        });
        return;
    ...
    let totalAdults = roomData.reduce((sum, r) => sum + Number(r.adults || 0), 0);
    if (totalAdults === 0) {
        alert('No of Pax Empty');
        return;
    }
    ...
    // Check length (typical mobile numbers are 10 digits)
    if (Mobile.length > 10 ) {
        alert("Mobile number should be 10 digits !");
        return false;
    }
    
    // Specific country validations (examples)
    
    // // For Indian numbers (starts with 6,7,8,9)
    // if (/^[6-9]\d{9}$/.test(Mobile)) {
    //     alert("Valid Indian mobile number");
    //     return true;
    // }
    
    // For US numbers (10 digits)
    // if (/^\d{10}$/.test(Mobile)) {
    //     alert("Valid US mobile number");
    //     return true;
    // }
    
    // International format (+ followed by digits)
//     if (/^\+[1-9]\d{1,14}$/.test(Mobile)) {
//    alert("Valid international mobile number");
//         return true;
//     }
...
// });
    ...
    success: function (result) {
        ...
        if (data.status === 'success' && data.roomgrcids.length > 0) {
       
            const allIds = data.roomgrcids.join(',');

            if (Number(printenable) !== 0 && printenable !== '') {
                swal({
                    title: "Check-in Saved Successfully!",
                    text: "Do you want the Registration Card?",
                    icon: "success",
                    buttons: true,
                    dangerMode: true
                }).then((willPrint) => {
                    if (willPrint) {
                        window.location.href = "<?php echo scs_index ?>Transaction/RegistrationCard?roomgrcid=" + allIds;
                    } else {
                        location.reload();
                    }
                });
            } else {
                swal("Success...!", "Check-in saved successfully!", "success").then(() => {
                    location.reload();
                });
            }

        } else {
            swal("Error...!", data.message || "Check-in save failed!", "error").then(() => {
                location.reload();
            });
        }
    ...
    error: function () {
        swal("Error", "Failed to save check-in (AJAX error).", "error");
    }
});

            } else {
                alert("No availability data received from server.");
            }
        },
        error: function () {
...
function Adult(element) {
    ...
    const rowIndex = row.getAttribute('data-row'); // e.g. 1, 2, 3...

    // get male/female/child/totalocc elements for this row
    const male = parseInt(row.querySelector(`#male_${rowIndex}`)?.value || 0);
    const female = parseInt(row.querySelector(`#female_${rowIndex}`)?.value || 0);
    const child = parseInt(row.querySelector(`#child_${rowIndex}`)?.value || 0);
    const totalocc = parseInt(document.getElementById("totalocc")?.value || 0);

    const totalPax = male + female + child;

    if (totalPax > totalocc) {
        alert(`Pax (${totalPax}) exceed room occupancy limit (${totalocc}).`);
        row.querySelector(`#Adults_${rowIndex}`).value = 0;
        return;
    }

    // update Adults field for this row
    row.querySelector(`#Adults_${rowIndex}`).value = totalPax;
}
</script>
...
const getratetyperoomrent = (planid)=>{
var count = document.getElementById('count').value;
var roomid = document.getElementById('RoomNo_' + count).value;

	$.ajax({
            type: 'get',
            url: "<?php echo scs_index ?>Transaction/getratetyperoomrentt?planid="+planid+"&Roomid="+roomid+"&fromdate=<?php echo $fromdate;?>&todate=<?php echo $todate;?>",
            success: function (html) {
              $("#rentdetails").html(html);
			   }			
          });
...
document.addEventListener('change', function(e) {
  if (e.target && e.target.name.startsWith('RoomNo_')) {
    checkDuplicateRoom(e.target);
  }
});

function checkDuplicateRoom(currentSelect) {
  ...
  document.querySelectorAll('select[name^="RoomNo_"]').forEach(select => {
  ...
  if (isDuplicate) {
    swal({
      title: "Duplicate Room Selected!",
      text: "This room number is already selected in another row!",
      icon: "warning",
      button: "OK",
    }).then(() => {
...
function getcity(val) {
    ...
    $.ajax({
        url: "<?php echo scs_index; ?>Auto_c/getcity",
        type: "POST",
        dataType: "json",
        data: {
            term: value
