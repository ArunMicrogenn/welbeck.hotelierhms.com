...
<div class="col-sm-12">
  ...
  <?php
     ...
     $fromdate = date('Y-m-d');
     $sql="  select  mas.checkindate,mas.checkintime,mas.ExpDate,mas.yearprefix, mr.roomno,mrt.RoomType_id,mt.Title+'.'+mc.Firstname+' '+mc.Lastname as Guestname, isnull(mcom.company,' ') 
    as company, mas.grcid,rs.Roomid,rs.roomgrcid, mas.noadults
    from trans_checkin_mas mas inner join room_status rs on mas.grcid=rs.grcid 
    inner join mas_room mr on mr.Room_Id = rs.Roomid 
    inner join Trans_RoomCustomer_det mrc on mas.grcid = mrc.grcid
