...
<?php   
   $Resid=$_POST['Room_id'];
   $sql="select ct.Cityid,* from Trans_Reserve_Mas mas
   ...
   $Adults=$rows->Adults; 
   $ResNo=$rows->ResNo;
   $ResDetid= $rows->resdetid;
   $Extrabedcount=$rows->Extrabedcount;   
   $Indate=date('Y/m/d', strtotime($rows->fromdate));
   $Indate1=date('Y/m/d', strtotime($rows->fromdate));
   $todate=date('Y/m/d', strtotime($rows->todate));
  ...
  $deleqry =$this->db->query($delete);
   while ($Indate1 <= $todate) 
		...
		where RM.Room_Id ='".$Roomid."'";
		 ...
		 values('".$Indate1."','".$Roomid."','".$singlerent."','".$Doublerent."','".$Triplerent."','".$Quartertriplerent."','I','".$singlerent."','".$Doublerent."','".$Triplerent."','".$Quartertriplerent."','".$row6['Extrabedamount']."','".User_id."',dbo.GrcNo(),'".Hotel_Id."')";
		 $result = $this->db->query($ins3);
		 $Indate1 = date ("Y/m/d", strtotime("+1 day", strtotime($Indate1)));
		  }
	    } 
?>
...
<table class="FrmTable T-6" style="margin-top:5px">
  <tr>
    ...
    <td><input type="date" name="Indate" readonly id="Indate" value="<?php echo date('Y-m-d' , strtotime($Indate)); ?>"  class="m-ctrl" style="background-color:#FFF59B;width:100px" ></td>
    <td>Hr</td>
	...
	<td><input type="date" readonly name="outdate" id="outdate"  value="<?php echo date('Y-m-d' , strtotime($todate)); ?>" style="background-color:#FFF59B;width:100px"  class="m-ctrl" ></td>	  
	<td>Hr</td>
	...
	$date1= date('Y-m-d' , strtotime($Indate));
	...
	{ $dateDiff ='1';}
	 ...
	 ?>
	  <td><input type="text" name="rnight" value="<?php echo $dateDiff; ?>" style="background-color:#FFF59B;width:50px" num=1 class="m-ctrl" onchange="Numberofnights(This.Value)" id="rnight"></td>
...
<table class="FrmTable" style="margin-top:-15px">
  ...
  <tr>
   <td><select readonly name="RoomNo" id="RoomNo" class="m-ctrl" >
    <?php   
   $Res=$this->Myclass->Room($rows->Roomid); 
   foreach($Res as  $row)
   {
	   echo '<option selected value='.$row['Room_Id'].'>'.$row['RoomNo'].'</option>';
   }
    ...
    <td><select onchange='Adults()' name="male" id="male" class="m-ctrl">
		...
		for($i=1; $i<=$totalpax;$i++)
		{
			...
			echo '<option '.$selected.' values='.$i.'>'.$i.'</option>';
		}
	...
	?>
	 ...
	 <td><input style="width:80px" readonly type='text' value='<?php echo $rows->noofpax; ?>' class="m-ctrl" id="Adults" name="Adults" >  </td>
...
<table class="FrmTable" style="margin-top:-15px" >
  ...
  <tr>
    <td><input name="Mobile" id="Mobile" value="<?php echo $rows->Mobile ?>" placeholder="Mobile"  oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="m-ctrl" ></td>
    ...
    <td  ><input name="Firstname" value="<?php echo $rows->Firstname ?>" id="Firstname" placeholder="First Name"  class="m-ctrl" ></td>
    <td><input name="Middlename"  id="Middlename" placeholder="Middle Name"  class="m-ctrl" ></td>
	<td><input name="Lastname" value="<?php echo $rows->Lastname ?>" id="Lastname" placeholder="Last Name"  class="m-ctrl" ></td>
...
<table class="FrmTable" >
  ...
  <tr>
	...
	<td><input type="text" value="<?php echo $rows->City;  ?>" placeholder="City" id="City" name="City" class="m-ctrl " /></td>
	<td><input name="Email_ID" value="<?php echo $rows->Email_ID ?>" id="Email_ID" placeholder="Email"  class="m-ctrl" ></td>
	<td><input name="Company" id="Company" placeholder="Company"  class="m-ctrl" ></td>
	<input type="hidden" id="Company_Id" name="Company_Id" value="">
	<input type="hidden" id="State_id" value="<?php echo $rows->State_id;  ?>" name="State_id" value="">
	<input type="hidden" id="City_id" value="<?php echo $rows->Cityid;  ?>" name="City_id" value="">
	<input type="hidden" id="Country_id" name="Country_id" value="">
	<input type="hidden"id="ResNo" name="ResNo" value="<?php echo $ResNo; ?>" >
...
<table class="FrmTable" style="margin-top:-25px" >
  <tr>
    ...
    <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
    <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
	<td><a id="Reserve" onclick="CheckIn()"  name="Reserve" class="btn btn-warning btn-sm" ><i class="fa fa-book" aria-hidden="true"></i> CheckIn </a></td>
  </tr>
</table>
...
$("#City").autocomplete({
    ...
    $.ajax({
			 ...
			 success:function(result){
					...
					if(result =='Sucess')		
						{
							swal("Sucess...!", "Checkin Save Sucessfully...!", "success")
							.then(function() {
								location.reload();	
