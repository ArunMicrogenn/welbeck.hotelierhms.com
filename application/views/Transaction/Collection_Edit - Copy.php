  ...
  <div class="the-box F_ram">
    ...
    <table class="table table-bordered table-hover"  >         
		   <tbody>
		    <?php 
			 $i=1;
		        $qry="select * from Trans_Checkout_mas cmas
			 ...
			 $advance= $exec->num_rows();
			  ...
			  {	
          if($rows['totalamount'] != $rows['Paidamount']){

          
        $totalamount = $totalamount+$rows['totalamount']-$rows['Paidamount'];			
				echo '<tr>';		 
				echo '<td  style="text-align: center;">'.$i.'</td>';
				echo '<td style="text-align: left;">'.$rows['Checkoutno'].'</td>';
        ...
        echo '<td style="text-align: left;">'.$rows['Noofpersons'].'</td>';
                ...
                <input type="text" class="f-ctrl rmm" readonly  num=1 id="Camt_'.$i.'" value="'.$rows['totalamount']-$rows['Paidamount'].'" required ></td>
                </td>';
        echo '<td style="text-align: left;">
            ...
            <?php 
             $qrys="select mc.Company_id ,mc.Company,Sum(isnull(pd.Amount,0)) - Sum(isnull(pd.Paidamount,0)) as totalamount from mas_company mc
              ...
              <tr>
                ...
                <td><input type="text" class="scs-ctrl Dat rmm"   value="<?php echo date('d-m-Y')?>" name="validate[]" id="validate_1"></td>
                <?php if($j==1){ ?>
...
<script>
 ...
 function paymodeValid(){
  ...
  $.ajax({
      ...
      success: function (result) {
        if(result =='success')		
      {
        swal("Success...!", "Collection Save Successfully...!", "success")
        .then(function() {
 ...
 function savee(a,i){
  ...
  function save(a,i){
    ...
    var Camt = document.getElementById("Camt_"+i).value;
    var amt_total = 0;
      ...
      var savebtn = document.getElementById("EXEC");  
        $(".add-row").click(function(){  
          ...
          i=i+1;
            ...
            var paymode="<td ><select  class='f-ctrl' onchange='paymodevalidate(this.value,i)' name='paymode[]' id='paymode_"+i+"' ><option value='0'>--Paymode--</option> <?php $Res=$this->Myclass->PayMode(); foreach($Res as $row) { if($row['InActive'] ==0 && $row['PayMode'] !='COMPANY') {  echo '<option value='.$row['PayMode_Id'].'>'.$row['PayMode'].'</option>'; }} ?> </select></td>";
