...
$this->pweb->Cheader('Transaction','Reservation Room Cancel');
    ...
    <?php
      if(@$_POST['frmdate']){
        ...
        if(@$_POST['bookingdate'] !=0){
            $sql="select isnull(rd.cancelrooms,0) as noshows, isnull(rd.checkinrooms,0) as checkinroom, 
            (rd.noofrooms-isnull(rd.checkinrooms,0)-isnull(rd.cancelrooms,0)) as Rooms,
            mr.RoomType,mt.title+'.'+mc.Firstname+' '+isnull(mc.Lastname,'') as Guestname, 
             * from Trans_Reserve_Mas rm 
            ...
            where  isnull(rd.stat, '')<> 'C' and isnull(rd.noshows,0)<>1 and
            rm.Resdate between '".$fromdate."' and '".$todate."'
            order by rm.ResNo";
        }else{
            $sql="select isnull(rd.cancelrooms,0) as noshows, isnull(rd.checkinrooms,0) as checkinroom, 
            (rd.noofrooms-isnull(rd.checkinrooms,0)-isnull(rd.cancelrooms,0)) as Rooms,
            ...
            inner join mas_title mt on mt.Titleid =mc.Titelid
            where  isnull(rd.stat, '')<> 'C' and isnull(rd.noshows,0)<>1 and
            rd.fromdate between '".$fromdate."' and '".$todate."'
            order by rm.ResNo";
        }	
       ...
       <th>Rooms</th>
            ...
            <form id="reserveform<?php echo $count;?>" method="POST">
			  <tr  class="tb">
                ...
                <input type="hidden"  num=1 name="resdetid" id="resdetid<?php echo $count;?>"  value="<?php echo $row1['resdetid']; ?>" class="f-ctrl rmm" >
                <td style="text-align:center"><input type="text"  readonly value="<?php echo $count;?>" class="f-ctrl rmm"  /></td>                                      
                <td><input type="text"  readonly value="<?php echo date('d-m-Y', strtotime($row1['Resdate']));?>" class="f-ctrl rmm"  /></td> 
                <td>
                <input type="text"  readonly value="<?php echo $row1['yearprefix'].'/'.$row1['ResNo']?>" class="f-ctrl rmm"  />
                </td>
                <td>
                ...
                <td>
                <input type="text"  readonly value="<?php echo date('d-m-Y', strtotime($row1['fromdate'])); ?>" class="f-ctrl rmm"  />
                </td>
                <td>
                <input type="text"  readonly value="<?php echo date('H:i', strtotime($row1['fromtime'])); ?>" class="f-ctrl rmm"  />
                </td>
                <td>
                <input type="text"  readonly value="<?php echo date('d-m-Y', strtotime($row1['todate'])); ?>" class="f-ctrl rmm"  />
                </td>
                <td>
                <input type="text"  readonly value="<?php echo date('H:i', strtotime($row1['totime'])); ?>" class="f-ctrl rmm"  />
                </td> 
                <td><input type="text"  name="checkinrooms" readonly value="<?php echo $row1['checkinroom'] ?>" class="f-ctrl rmm"  /></td>
                <td>
                <input type="text" name="cancelrooms" readonly value="<?php echo $row1['noshows']; ?>" class="f-ctrl rmm"  />
                </td>
                <td>
                <input type="text"  name="Rooms" id="Rooms<?php echo $count; ?>" num=1 maxlength="1"  oninput="this.value = this.value.replace(/[^0-<?php echo $row1['Rooms'];?>$?>.]/g, '').replace(/(\..*)\./g, '$1');"  value="" class="f-ctrl rmm"  />
                </td>
                <td>
...
<SCRIPT language="javascript">
	 ...
	 function Reserve(a)
	{
        ...
        const Rooms = document.getElementById("Rooms"+a).value;
           if(Rooms== 0)
             { swal("Unable to process", "Rooms is Empty", "error"); 
               return; }
            if(reason=='')
    ...
    $.ajax({
      type: 'post',
      url: '<?php echo scs_index ?>Transaction/ReservetionRoomcancel',
        ...
        if(result =='Success')		
      {
        ...
        .then(function() {
          window.location.href='<?php echo scs_index?>Transaction/ResRoomCancelList';	
          });
      }
      else
      {
