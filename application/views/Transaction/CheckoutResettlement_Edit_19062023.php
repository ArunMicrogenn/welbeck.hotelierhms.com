...
where rs.status <>'N' and rs.billsettle<>1 and Checkoutid='" . @$ID . "'";
            ...
            foreach ($ress->result() as $rows) { ?>
              <tr>
                    ...
                    foreach ($Res as $row) {
                      if ($row['InActive'] == 0 && $row['PayMode']) { ?>
                        <option <?php if ($rows->Paymodeid == $row['PayMode_Id']) {
                          ...
                          value="<?php echo $row['PayMode_Id'] ?>"><?php echo $row['PayMode'] ?></option>
                        <?php
                      }
                    }
                    ?>
                ...
                <td><input type="text" class="scs-ctrl Dat rmm"
                    value="<?php echo date("d-m-Y", strtotime($rows->Validdate)); ?>" value='<?php echo date('d-m-Y') ?>'
  ...
  $(document).ready(function () {
    ...
    for (let j = 1; j <= rowCount - 1; j++) {
      ...
      if (Number(paymode) == 1) {
        ...
        //  } else if(Number(paymode) == 6){
        // $.ajax({
        //   type:"POST",
        //   url :"<?php echo scs_index ?>Transaction/toroomSettleedit",
        //   data:'no='+'<?php echo $Checkoutno; ?>',
        //   success: function (html){
        //     $("#bank_"+i).html(html)
        //       }
        // })
