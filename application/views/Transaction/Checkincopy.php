...
<?php
  ...
  {  $DateofAudit=$row['DateofAudit'];}
   while ($Indate <= $todate) 
		{
echo	$sql6="SELECT * FROM Mas_RatePlan RP 
		left JOIN Mas_RatePlan_Det RD on RD.RatePlan_Id=RP.RatePlan_Id
		...
		where RM.Room_Id ='".$Roomid."'";
		  ...
		  { $Adults=$row6['Adults']; 
			...
			// for($i=1;$i<=$no; $i++){
			// 	echo $ins3=$ins3."Insert into temp_trans_roomdet_det_rent(Rentdate,roomid,singlerent,Doublerent,Triplerent,Quartertriplerent,checkinrenttype,actsinglerent,actdoublerent,acttriplerent,actquartertriplerent,extraadultcharges,userid,GrcNo,hotelid)
			// 	values('".$Indate."','".$Roomid."','".$row6['Single'][$i]."','".$row6['Doubles'][$i]."','".$row6['Triple'][$i]."','".$row6['Quadruple'][$i]."','I','".$row6['Single'][$i]."','".$row6['Doubles'][$i]."','".$row6['Triple'][$i]."','".$row6['Quadruple'][$i]."','".$row6['Extrabedamount'][$i]."','".User_id."',dbo.GrcNo(),'".Hotel_Id."')";
			// 	$result = $this->db->query($ins3);
			// }

			echo $ins3="Insert into temp_trans_roomdet_det_rent(Rentdate,roomid,singlerent,Doublerent,Triplerent,Quartertriplerent,checkinrenttype,actsinglerent,actdoublerent,acttriplerent,actquartertriplerent,extraadultcharges,userid,GrcNo,hotelid)
			 	values('".$Indate."','".$Roomid."','".$singlerent."','".$Doublerent."','".$Triplerent."','".$Quartertriplerent."',
				'I','".$singlerent."','".$Doublerent."','".$Triplerent."','".$Quartertriplerent."','".$row6['Extrabedamount'][$i]."','".User_id."',dbo.GrcNo(),'".Hotel_Id."')";
				$result = $this->db->query($ins3);
...
<table class="FrmTable T-6" style="margin-top:5px">
  <tr>
    ...
    <td ><input type="date" name="" id="Indate" style="background-color:white;" value="<?php echo $_POST['Fdat']; ?>"  min="<?php echo date('Y-m-d');?>" onChange="outdate();" class="m-ctrl"  style="background-color:#FFF59B;width:100px"  ></td>
    <td>Hr</td>
	...
	<td><input type="date" name="" id="outdate" style="background-color:white;" value="<?php echo $_POST['Tdat']; ?>"  style="background-color:#FFF59B;width:100px" onChange="rnight();" class="m-ctrl"></td>	  
	<td>Hr</td>
...
function outdate() {
	...
	document.getElementById("rnight").value = daydiff;
...
$("#City1").autocomplete({
	...
	{ alert('Please Select the City'); 
		return; }
    $.ajax({
		     url:"<?php echo scs_index ?>Transaction/Checkinsave?Roomid="+Roomid+"&Title="+Title+"&Indate="+Indate+"&todate="+todate+"&Mobile="+Mobile+"&Firstname="+Firstname+"&Lastname="+Lastname+"&RateCode="+RateCode+"&City_id="+City_id+"&State_id="+State_id+"&Country_id="+Country_id+"&Email_ID="+Email_ID+"&Child="+Child+"&Adults="+Adults+"&FHr="+FHr+"&FMi="+FMi+"&THr="+THr+"&TMi="+TMi+"&male="+male+"&female="+female+"&Middlename="+Middlename+"&Company_Id="+Company_Id+"&guestaddress="+guestaddress+"&companyaddress="+companyaddress+"&travelagent_Id="+travelagent_Id+"&bookingid="+bookingid+"&discper="+discper+"&discamt="+discamt+"&foodplan="+foodplan,
         	 type: "POST",
			 dataType: "html",
			 success:function(result){
					$('.ui-dialog-titlebar-close').click();		
					if(result =='Sucess')		
						{
							swal("Success...!", "Checkin Save Successfully...!", "success")
							.then(function() {
    							location.reload();	
								});
						}
