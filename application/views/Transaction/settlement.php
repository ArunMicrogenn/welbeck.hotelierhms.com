 ...
 $time= date("H:i:s");
  ...
  $sql = "
    SELECT distinct
        ...
        ISNULL(rmas.Amount, 0) AS receipt,
        cmas.Roomid,
        cmas.roomgrcid,
        cmas.totalamount,
        cmas.yearPrefix,
    ...
    WHERE cmas.Checkoutno LIKE '%CHK%' AND cmas.Checkoutid = '$billId'
";
...
if ($row) {
    ...
    $Mobile      = $row['Mobile']         ?? '';
    $Billno      = $row['Billno']         ?? '';
    $Add1        = $row['HomeAddress1']   ?? '';
    $Add2        = $row['HomeAddress2']   ?? '';
