...
<?php
 ...
 foreach ($res->result_array() as $row)
 {
	 ...
	 $grcid=$row['grcid']; $roomgrcid=$row['roomgrcid'];  $City=$row['City']; $Nationality=$row['Nationality'];
	  $Mobile=$row['Mobile']; $Noofpersons=$row['persons']; $idprooftype = $row['Id_Documenttype']; $idproofno = $row['Id_Documentno']; 
   ...
   $rent1="select * from Trans_Roomdet_det_rent Where  roomgrcid='".$roomgrcid."' and Rentdate=(select DateofAudit from Night_Audit)";
 ...
 { 
	...
	else if($Noofpersons >=4)
	{
		...
		{
		$triple = 	$rentrow1['Triple'] ?? 0;
			$Actrackrate = $triple + $rentrow1['extraadultcharges'];		
		}
		else
		{
  ...
  ?>
  	...
  	<?php 
	   $id = "SELECT * FROM mas_idproof";	
        $qry = $this->db->query($id);
		$proofid = $qry->result_array();
	 ?>
	...
	<div class="modal-content" style="width:60%">
	   <div class="ui-dialog-titlebar ui-corner-all ui-widget-header ui-helper-clearfix ui-draggable-handle">
		...
		<table class="FrmTable" >
		 ...
		 <tbody>
		 <?php $i=1;
		     $rent="select * from Trans_Roomdet_det_rent Where  roomgrcid='".$roomgrcid."' order by Rentdate";
		 ...
		 <?php $i++; } ?>
		   ...
		   <input type="hidden" id="tariffroomid" name ="tariffroomid" value="<?php echo $Roomid ?>">
		   <input type="hidden" id="tariffroomgrcid" name ="tariffroomgrcid" value="<?php echo $roomgrcid ?>">
		 </tbody>
		</table>
	</div>
	</div>
...
<form id="Guestamendmentsave" method="POST" enctype="multipart/form-data">
    <div id="GuestDetails" class="tabcontent">
		 ...
		 <tr>
			<td>Title</td>
		<?php $seltitle = "select * from mas_title";
		  $titexe = $this->db->query($seltitle)->result_array();  ?>
		<td><select name="title" id="title">
			<?php  foreach($titexe as $tit) { ?>
			<option value="<?php echo $tit['Titleid'] ?>" <?php echo (isset($Titleid) && $Titleid == $tit['Titleid']) ? 'selected' : '' ?>><?php echo $tit['Title']?></option>
			<?php } ?>
		</select></td>
			</tr>
	   <tr>
	   ...
	   <tr>
	<td>Mobile Number</td>
		<td><input name='Mobile' type='text' class="m-ctrl" id="Mobile" value="<?php echo $Mobile; ?>"></td>
		<td>Phone</td>
 ...
 <select name="Id_Documenttype" id="Id_Documenttype" class="m-ctrl">
    <?php 
        foreach ($proofid as $proof) { 
            $selected = ($proof['proofid'] == $idprooftype) ? 'selected' : '';
    ?>
        <option value="<?php echo $proof['proofid']; ?>" <?php echo $selected; ?>>
            <?php echo $proof['idname']; ?>
        </option>
    <?php 
        }
    ?>
				...
				<td>ID ProofNo</td>
				<td><input name='Id_Documentno' id="Id_Documentno" type='text' class="m-ctrl"  value="<?php echo $idproofno; ?>"></td>
  ...
  <table class="FrmTable" style="width:100%">
	   <tr>
		...
		<?php
			...
			// $qry2 = "SELECT * FROM Mas_RatePlan RP 
			...
			// where RM.Room_Id ='".$Roomid."'  and isnull(RP.act,0)=0 and PT.PlanType_Id = '".$PlanType_Id."' Order by PT.PlanType_Id";
			$qry2="select * from Mas_plantype ";
			...
			{ 
				...
				?>
		<option value="<?php echo $row2['PlanType_Id'] ?>" 
    <?php echo (isset($PlanType_Id) && $PlanType_Id == $row2['PlanType_Id']) ? 'selected' : '' ?>>
    <?php echo $row2['RateCode'] ?>
</option>
			<?php 
			}
			?>
...
<?php $qry = "
select trdet.planid 
from trans_roomdet_det trdet
inner join room_status rs on rs.roomgrcid = trdet.roomgrcid
...
$savedPlanId = $res['planid'] ?? ''; ?>
		...
		<?php
			...
			foreach ($res2->result_array() as $row2)
			{
				$selected = ($row2['FoodPlan_Id'] == $savedPlanId) ? 'selected' : '';
				echo '<option value="'.$row2['FoodPlan_Id'].'" '.$selected.'>'
					 .$row2['FoodPlan'].'</option>';
			}
			?>
		   </select></td>
		   <?php 
		...
		<td><input type="hidden" id="DISA" value="<?php echo $disamount;  ?>"><input style="text-align:right" onchange="discamt1()" name="discamt" id="discamt" value="<?php echo @$discAmount ?>" placeholder="Disc.Amt"  class="m-ctrl" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" ></td>
		  ...
		  <tr>
			<?php  $reason="select Reason from trans_roomdet_det where roomgrcid='".$roomgrcid."'";
		 $resry=$this->db->query($reason)->row_array(); ?>
		  <td id="discrslab">Discount Reason</td>
	
		  <td id="discrs" >
  <input type="hidden" id="DISR" value="<?php echo $resry['Reason']; ?>"><textarea  name="discreason"  oninput="checkDiscountBeforeReason()" id="discreason"  placeholder="Disc. Reason" class="m-ctrl"
  style="width:100%; height:60px;"><?php echo $resry['Reason']; ?></textarea></td>
		<td></td><td></td>
		<td></td><td><input type="button"  onclick="Guestamendment();" value="Save" class="btn btn-warning btn-sm"></td>
	   </tr>
	</table> 
...
<div id="Otherdetails" class="tabcontent">
   <table class="FrmTable" style="width:100%">
	   <tr>
		...
		<td><select name="Company_id" id="Company_id" class="m-ctrl">
		...
		<?php
			...
			$comqry = "select * from mas_company";
			$comexe= $this->db->query($comqry);
			
			foreach($comexe->result_array() as $comp)
			{ ?>
			<option value="<?php echo $comp['Company_Id']  ?>"><?php echo $comp['Company'] ?></option>
...
// }
  ...
  const reasonField = document.getElementById('discreason').value;

  if (discPer > 0 || discAmt > 0) {
	if(reasonField == ''){
		swal("Warning...!", "Please Enter Reason...!", "warning").then(()=>{
						// location.reload();	
					})
					return
	}
          ...
          $.ajax({
            ...
            success: function (result) {
				...
				if(result.trim() == 'success'){
					swal("Success...!", "Guestamendment Save Successfully...!", "success").then(()=>{
						location.reload();	
					})
				}else{
					swal("unable to process...!", result, "warning").then(()=>{
						location.reload();	
					});
				}
				/*swal({
			...
			$.ajax({
		type: 'get',
		url: "<?php echo scs_index ?>Transaction/GuestImageSlider?id="+id,
		dataType: "html",
		success: function (html) {
			var modalImg = document.getElementById("GuestImageModal");
			modalImg.style.display = "block";
			$('#gusetimageslider').html(html);
...
<script>
function checkDiscountBeforeReason() {
  const discPer = parseFloat(document.getElementById('discper').value) || 0;
  const discAmt = parseFloat(document.getElementById('discamt').value) || 0;
  const reasonField = document.getElementById('discreason');

  if (discPer <= 0 && discAmt <= 0) {
	
    alert("Discount % or Amount must be greater than 0 before entering a reason.");
    reasonField.value = '';
    reasonField.blur(); 
    return false;
  } 
