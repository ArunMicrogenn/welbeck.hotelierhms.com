...
class ReservationAdvances extends CI_Model
{
	...
	function ReservationAdvances_Val()
	{
		...
		//  $this->form_validation->set_rules('typeid', 'roomtypee', 'required');
	...
	function ReservationAdvances_exec()
	{  if($_REQUEST['BUT'] =='SAVE')
	   {	
		 ...
		 else 
	     {
			...
			$qry="exec Exec_ResAdvance '".$_REQUEST['roomid']."' ,'".$_REQUEST['Amount']."','".$_REQUEST['paymode']."','".date('Y-m-d')."','".@$_REQUEST['bank']."','".@$_REQUEST['cardnumber']."','".@$_REQUEST['validate']."',".User_id.",'".$_REQUEST['Remark']."','".$_REQUEST['idv']."','".str_replace(" ","",$_REQUEST['BUT'])."' ";	  
